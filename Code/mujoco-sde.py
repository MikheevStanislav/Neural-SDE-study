...
def main(
    ...
    **kwargs
    ...
    # data loader
    times, train_dataloader, val_dataloader, test_dataloader = dataset.get_data(
    ...
    folder_name = dataset_name.capitalize() + '_' + str(missing_rate)
    test_name = "step_" + "_".join([ str(j) for i,j in dict(vars(args)).items()]) + "_" + str(experiment_id)
    result_folder = PATH + '/tensorboard'
    writer = SummaryWriter(f"{result_folder}/runs/{folder_name}/{str(test_name)}")
    #model initialize
    make_model = common.make_model(model_name, input_channels, num_output_features, hidden_channels,
                                   ...
                                   sde_initialization_seed=manual_seed)
    ...
    if dry_run:
        ...
        name = dataset_name.capitalize() + '_' + str(missing_rate)
